...
public:
    ...
    // Reprocess an existing file toggling encryption/compression settings.
    // Reads current plaintext, then applies new settings and updates DB.
    bool reprocessFile(int fileId, bool encrypt, bool compress);

    // Preferences: default algorithms and compression level
...
private:
    ...
    // Defaults used when processing content
